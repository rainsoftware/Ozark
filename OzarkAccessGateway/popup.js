...
(function () {
  ...
  const handleTerminalInput = (term) => {
    ...
    term.onData((data) => {
      if (data === '\r') {
        ...
        } else if (command.startsWith('piston ')) {
          const code = command.slice('piston '.length).trim();
          if (code) {
            executeWithPiston(code, term);
          } else {
            term.writeln('Error: No code provided. Usage: piston <code>');
          }
        } else {
  ...
  const executeWithPiston = (code, term) => {
    fetch('https://emkc.org/api/v2/piston/execute/', {
      ...
      .then((data) => {
        if (data.output) {
          term.writeln('Piston Output:\n' + data.output);
        } else if (data.stderr) {
          term.writeln('Piston Error:\n' + data.stderr);
        } else {
          term.writeln('Piston: No output.');
        }
      })
      .catch((err) => {
        term.writeln('Piston execution error: ' + err.message);
      });
  };
